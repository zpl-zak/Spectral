...
//# include <windows.h>
#   include <xinput.h>
//# pragma comment(lib, "xinput") //< @r-lyeh
...
#   include <libudev.h>
#else
//# error "Unknown platform in 3rd_gamepad.h"
#endif

#define BUTTON_TO_FLAG(b) (1 << ((b)-1))
...
XInputEnable_t XInputEnable_ = NULL;
XInputGetState_t XInputGetState_ = NULL;
XInputSetState_t XInputSetState_ = NULL;
...
void GamepadInit(void) {
    ...
    if(!ever) {
        ...
        h = h ? h : LoadLibraryA("xinput9_1_0.dll"); // win vista, 7... shipped. more limited than 1_3
        if( h ) {
            ...
            XInputGetState_ = (XInputGetState_t)GetProcAddress(h, (LPCSTR)100);
            if (!XInputGetState_)
            XInputGetState_ = (XInputGetState_t)GetProcAddress(h, "XInputGetState");
...
static void GamepadUpdateDevice(GAMEPAD_DEVICE gamepad) {
    XINPUT_STATE xs;
    if (XInputGetState_ && XInputGetState_(gamepad, &xs) == 0) { //< @r-lyeh
...
void GamepadSetRumble(GAMEPAD_DEVICE gamepad, float left, float right) {
    ...
    if ((STATE[gamepad].flags & FLAG_RUMBLE) != 0) {
        ...
        XInputSetState_(gamepad, &vib); //< @r-lyeh
    }
}
